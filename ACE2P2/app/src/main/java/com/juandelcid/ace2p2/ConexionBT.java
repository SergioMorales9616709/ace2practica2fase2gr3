...
public class ConexionBT {
    ...
    private synchronized void setState(int estado) {
        ...
        mHandler.obtainMessage(MainWindow.Mensaje_Estado_Cambiado, estado, -1).sendToTarget();
    }
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = mHandler.obtainMessage(MainWindow.Mensaje_Nombre_Dispositivo);
        Bundle bundle = new Bundle();
        bundle.putString(MainWindow.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(MainWindow.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(MainWindow.TOAST, "Error de conexi?n");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(MainWindow.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString( MainWindow.TOAST, "Se perdio conexion");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
        msg = mHandler.obtainMessage(MainWindow.MESSAGE_Desconectado);
        mHandler.sendMessage(msg);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(MainWindow.Mensaje_Escrito, -1, -1, buffer).sendToTarget();
            } catch (IOException e) {
        ...
        public void run() {
            ...
            while (true) { //Mantiene escuchando el InputStream mientras este conectado
                try {
                    ...
                    mHandler.obtainMessage( MainWindow.Mensaje_Leido, bytes, -1, buffer).sendToTarget(); //readMessageX por buffer
                } catch (IOException e) {
